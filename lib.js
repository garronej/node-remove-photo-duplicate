...
const image_extensions = new Set(require("./image_extensions"));
...
function remove_duplicate(root_dir_path, dry_run) {
    ...
    const build_map_rec = (dir_path, map) => {
        ...
        for (const file_name of fs.readdirSync(dir_path)) {
            ...
            if (fs.lstatSync(file_path).isDirectory()) {
                ...
                build_map_rec(file_path, map);

            } else {

                if (!image_extensions.has(path.extname(file_name).substr(1))) {

                    console.log(`${path.relative(root_dir_path, file_path)} is not a image, skipping`);
    ...
    const remove_dir_if_empty_rec = dir_path => {
        ...
        if (file_names.length !== sub_dir_paths.length) {
            return false;
        }
    ...
    build_map_rec(root_dir_path, map);
